...
export function WorkflowSidebar({
  ...
  return <aside className={collapsed ? "sidebar collapsed" : "sidebar"}>
    <div className="sidebar-main">
      ...
      {!collapsed && <p className="muted sidebar-note">{language === "zh" ? "本地优先 Web UI。报告结果前必须检查模型、范围、初值、收敛和 warnings。" : "Local-first Web UI. Check model, range, initials, convergence, and warnings before reporting results."}</p>}
    </div>
    <div className="sidebar-footer">
      ...
      </select></label> : <button className="language-icon" title={t(language, "language")} onClick={() => onLanguageChange(language === "en" ? "zh" : "en")}>🌐</button>}
