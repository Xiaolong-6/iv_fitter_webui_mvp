    ...
    case "data":
      return <ManualSection id="data" title={zh ? "3. 数据导入与 trace 选择" : "3. Data import and trace selection"}>
        {zh ? <>
          ...
          <ul className="doc-steps"><li>Supported input: CSV/TXT/DAT, pasted tables, generic voltage/current columns, and HappyMeasure single- or multi-trace exports.</li><li>Import CSV/TXT opens the demo IV traces folder by default when the local runtime supports it; you can still browse anywhere and import your own files.</li><li>Column detection is heuristic. Always verify units and sign convention in the preview table.</li><li>Multi-trace files preserve trace identity. Fit only the selected trace unless a future joint-fit workflow is explicitly used.</li><li>Look for missing rows, duplicate voltage points, compliance plateaus, instrument limits, and obvious wiring/sign reversals.</li></ul>
          <h3>Synthetic data generator</h3>
